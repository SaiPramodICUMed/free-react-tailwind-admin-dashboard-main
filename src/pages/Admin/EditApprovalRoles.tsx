...
const EditApprovalRoles: React.FC = () => {
    ...
    const addNewSegment = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `https://vm-www-dprice01.icumed.com:5000/api/approval/add`,
                payload,
    ...
    const deleteSegment = async () => {
        ...
        try {
            ...
            const payload = {
                approvalID: selectedApprovalObject.id,
            };

            console.log("deleteRole payload:", payload);

            const response = await axios.post(
                `https://vm-www-dprice01.icumed.com:5000/api/approval/delete`,
                payload,
    ...
    const renameColumnRole = async () => {
        ...
        try {
            const payload = {
                approvalID: selectedApprovalObject.id,
                newApprovalName: newApproval.trim(),
            };

            const response = await axios.post(
                `https://10.2.6.130:5000/api/approval/update`,
                payload,
                { headers: { "Content-Type": "application/json" } }
            );
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen text-sm relative">
            ...
            <div className="flex items-center justify-between mb-4">
                <h1 className="text-xl font-semibold text-gray-800">Edit Approval Roles</h1>
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-lg bg-white shadow-sm">
                <table className="min-w-full border-collapse text-gray-700">
                    <thead className="bg-[#0f59ac] text-white">
                        <tr>
                            <th className="px-4 py-2 border-b text-left font-medium">Approval Role Id</th>
                            <th className="px-4 py-2 border-b text-left font-medium">Approval Role Name</th>
                        </tr>
                    </thead>
                    <tbody>
                        {approvalRoles.length > 0 ? (
                            approvalRoles.map((seg: any, i: number) => {                               
...
export default EditApprovalRoles;
