...
export default function RecentlyLoaded() {
  ...
  // SQL Filter Builder
  const buildFilterString = (obj: any) => {
    const clauses = [];
    const base = `AND (1 <> 1 OR LoadStatus = 5)`;
    ...
    for (const key of Object.keys(obj)) {
      ...
      if (Array.isArray(val) && typeof val[0] === "string") {
        clauses.push("(" + val.map(v => `${key}='${escapeSql(v)}'`).join(" OR ") + ")");
        continue;
      }
  ...
  const fetchData = async (start: number, end: number, filterOverride?: string) => {
    ...
    try {
      const payload = {
        ...
        sortBy: "TaskId",
        sortByDirection: "asc",
        filter: filterOverride ?? `AND (1 <> 1 OR LoadStatus = 5)`,
        fieldList: "*",
        timeout: 0
      };
  ...
  // When BasicTables sends filters
  const searchData = (filtersObj: any) => {
    setFiltersState(filtersObj);
    const sql = buildFilterString(filtersObj);
  ...
  const setPageChange = (pageNumber: number, perPage?: number) => {
    ...
    const sql = buildFilterString(filtersState);
  ...
  useEffect(() => {
    loadMetadata();
    fetchCount(`AND (1 <> 1 OR LoadStatus = 5)`);
    ...
    <>
      ...
      <div className="grid grid-cols-6 gap-4 md:gap-3">
        ...
        <div className="col-span-12">
          <BasicTables
            page={"Pricing - ERP Load - Recently Loaded"}
            inboxData={inboxData}
            columns={columns}
