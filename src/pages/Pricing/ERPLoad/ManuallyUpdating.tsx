...
export default function ManuallyUpdating() {
  ...
  const buildFilterStringFromObject = (obj: Record<string, any>) => {
    ...
    const base = `AND (1 <> 1 OR LoadStatus = 3)`;
    ...
    for (const key of Object.keys(obj)) {
      ...
      if (Array.isArray(val) && val[0] instanceof Date) {
        if (val[0] && val[1]) {
          clauses.push(
            `(${key} >= '${formatDateForSql(val[0])}' AND ${key} <= '${formatDateForSql(val[1], true)}')`
          );
        }
        continue;
      }
  ...
  const fetchData = async (start: number, end: number, filterOverride?: string) => {
    ...
    try {
      const payload = {
        ...
        sortBy: "TaskId",
        sortByDirection: "asc",
        filter: filterOverride ?? `AND (1 <> 1 OR LoadStatus = 3)`,
        fieldList: "*",
        timeout: 0
      };
  ...
  useEffect(() => {
    loadMetadata();
    fetchCount(`AND (1 <> 1 OR LoadStatus = 3)`);
  ...
  return (
    <>
      ...
      <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-4">
        ...
        /
        <span className="text-gray-500 font-medium">&nbsp;ERP Load - Manually Updating</span>
      </nav>
      ...
      <div className="grid grid-cols-6 gap-4 md:gap-3">
        ...
        <div className="col-span-12">
          <BasicTables
            page={'Pricing - ERP Load - Manually Updating'}
            inboxData={inboxData}
            columns={columns}
