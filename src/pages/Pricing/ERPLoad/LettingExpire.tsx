...
export default function LettingExpire() {
  ...
  const buildFilterString = (obj: any) => {
    const clauses = [];
    const base = `AND (1 <> 1 OR LoadStatus = 4)`;
  ...
  const fetchData = async (start: number, end: number, filterOverride?: string) => {
    ...
    try {
      const payload = {
        ...
        sortBy: "TaskId",
        sortByDirection: "asc",
        filter: filterOverride ?? `AND (1 <> 1 OR LoadStatus = 4)`,
        fieldList: "*",
        timeout: 0
      };
  ...
  useEffect(() => {
    loadMetadata();
    fetchCount(`AND (1 <> 1 OR LoadStatus = 4)`);
  ...
  return (
    <>
      ...
      <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-4">
        ...
        /
        <span className="text-gray-500 font-medium">&nbsp;ERP Load - Letting Expire</span>
      </nav>
      ...
      <div className="grid grid-cols-6 gap-4 md:gap-3">
        ...
        <div className="col-span-12">
          <BasicTables
            page={"Pricing - ERP Load - Letting Expire"}
            inboxData={inboxData}
            columns={columns}
